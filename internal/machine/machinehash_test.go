...
func (s *ValidateMachineHashSuite) TestItFailsWhenAppAddressIsWrong() {
	...
	templateHash, err := ReadHash(machineDir)
	s.Require().Nil(err)

	ctx := context.Background()
	_, cleanup, err := ethutil.CreateAnvilSnapshotAndDeployApp(ctx, templateHash)
	s.Require().Nil(err)
	defer cleanup()
...
func (s *ValidateMachineHashSuite) TestItSucceedsWhenHashesAreEqual() {
	...
	ctx := context.Background()
	appAddr, cleanup, err := ethutil.CreateAnvilSnapshotAndDeployApp(ctx, templateHash)
	s.Require().Nil(err)
	defer cleanup()
