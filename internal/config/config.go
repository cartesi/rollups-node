...
func get[T any](name, defaultValue string, hasDefault bool, parser func(string) (T, error)) T {
	v, ok := getOptional(name, defaultValue, hasDefault, parser)
	if !ok {
		fail("missing required %s env var", prefix+name)
	}
