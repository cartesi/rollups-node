...
const (
	...
	// Producer: Claimer. Consumer: external (via PG LISTEN).
	// The Claimer uses SignalReschedule() for self-continuation.
	ChannelClaimSubmitted Channel = "claim_submitted"

	// ChannelClaimAccepted signals terminal epoch state.
	// Producer: Claimer, PRT. Consumer: external (via PG LISTEN).
	ChannelClaimAccepted Channel = "claim_accepted"

	// ChannelSettleSubmitted signals that a PRT settle transaction was sent to L1.
	// Producer: PRT. Consumer: external (via PG LISTEN).
	// The PRT service uses SignalReschedule() for self-continuation.
