...
func TestSubscriberCloseStopsListenAndClosesChannels(t *testing.T) {
	connStr := getTestConnString(t)
	...
	go func() {
		done <- sub.Listen(context.Background())
	}()
	...
	_, ok := <-ch
	assert.False(t, ok, "subscription channel should be closed after Close")
}
