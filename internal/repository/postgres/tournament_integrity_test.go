...
func TestTournamentLevelConstraints(t *testing.T) {
	for _, test := range []struct {
		name       string
		maxLevel   uint64
		level      uint64
		...
		{name: "single level root", maxLevel: 1, level: 0},
		{name: "multi level root", maxLevel: 3, level: 0},
		{name: "inner nonleaf", maxLevel: 3, level: 1},
		{name: "inner leaf", maxLevel: 2, level: 1},
		{name: "zero levels", maxLevel: 0, level: 0, constraint: "tournaments_max_level_check"},
		{name: "level equals count", maxLevel: 1, level: 1, constraint: "tournaments_max_level_gt_level_check"},
		{name: "level exceeds count", maxLevel: 1, level: 2, constraint: "tournaments_max_level_gt_level_check"},
	} {
		t.Run(test.name, func(t *testing.T) {
			r, app, tournament, match := newTournamentIntegrityFixture(t)
			if test.level > 0 {
				require.NoError(t, r.CreateMatch(t.Context(), app.Name, match))
				tournament = repotest.NewTournamentBuilder(app.ID).WithLevel(1).
					WithParent(tournament.Address, match.IDHash).Build()
				require.NoError(t, r.CreateTournament(t.Context(), app.Name, tournament))
			}
			kind := TournamentKindNonLeaf
			if test.maxLevel > 0 && test.level == test.maxLevel-1 {
				kind = TournamentKindLeaf
			}

			// Write directly to test the database constraint, not a Go-side guard.
			_, err := r.db.Exec(t.Context(), `UPDATE tournaments SET max_level = $1, level = $2, kind = $3
                WHERE application_id = $4 AND epoch_index = $5 AND address = $6`,
				test.maxLevel, test.level, kind.String(), app.ID, tournament.EpochIndex, tournament.Address.Bytes())
			if test.constraint != "" {
				var pgErr *pgconn.PgError
				require.ErrorAs(t, err, &pgErr)
				...
				require.NoError(t, err)
				tournament.MaxLevel, tournament.Level, tournament.Kind = test.maxLevel, test.level, kind
			}
			stored, err := r.GetTournament(t.Context(), app.Name, tournament.Address.Hex())
			require.NoError(t, err)
			require.Equal(t, tournament.MaxLevel, stored.MaxLevel)
			require.Equal(t, tournament.Level, stored.Level)
			require.Equal(t, tournament.Kind, stored.Kind)
		})
	}
}
