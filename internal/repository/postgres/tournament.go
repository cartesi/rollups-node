...
func (r *PostgresRepository) UpdateTournament(
	...
	finalState := postgres.NULL
	if t.FinalStateHash != nil {
		winnerCommitment = postgres.Bytea(t.FinalStateHash.Bytes())
	}
	...
	updateStmt := table.Tournaments.
		...
		WHERE(
			whereClause.
				AND(table.Tournaments.ApplicationID.EQ(postgres.Int(t.ApplicationID))).
				AND(table.Tournaments.EpochIndex.EQ(postgres.RawFloat(fmt.Sprintf("%d", t.EpochIndex)))).
				AND(table.Tournaments.Address.EQ(postgres.Bytea(t.Address))),
		)
