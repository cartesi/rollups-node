...
func (repository *AdvancerRepository) GetInputs(
	ctx context.Context,
    ...
    `, toIN(appAddresses)) // NOTE: not sanitized
...
func (repository *AdvancerRepository) StoreResults(
	ctx context.Context,
	input *Input,
	...
	// Inserts the outputs.
	nextOutputIndex, err := repository.getNextIndex(ctx, tx, "output", input.AppAddress)
	if err != nil {
		return err
	}
	err = repository.insert(ctx, tx, "output", res.Outputs, input.Id, nextOutputIndex)
	if err != nil {
		return err
	}

	// Inserts the reports.
	nextReportIndex, err := repository.getNextIndex(ctx, tx, "report", input.AppAddress)
	if err != nil {
		return err
	}
	err = repository.insert(ctx, tx, "report", res.Reports, input.Id, nextReportIndex)
	if err != nil {
		return err
	}

	// Updates the input's status.
	err = repository.updateInput(ctx, tx, input.Id, res.Status, res.OutputsHash, res.MachineHash)
	if err != nil {
		return err
	}
...
func toIN[T fmt.Stringer](a []T) string {
	s := []string{}
	for _, x := range a {
