...
func (s *InputReaderSuite) TestItEventuallyBecomesReady() {
	...
	select {
	case <-ready:
		s.repository.AssertNumberOfCalls(s.T(), "InsertInputsAndUpdateMostRecentFinalizedBlockNumber", 1)
	case err := <-errChannel:
...
func (s *InputReaderSuite) TestItReadsInputsFromNewBlocks() {
	...
	s.repository.AssertNumberOfCalls(s.T(), "InsertInputsAndUpdateMostRecentFinalizedBlockNumber", 2)
}
...
func (s *InputReaderSuite) TestItStartsWhenInputBoxBlockIsTheMostRecentFinalizedBlock() {
	...
	s.repository.AssertNumberOfCalls(s.T(), "InsertInputsAndUpdateMostRecentFinalizedBlockNumber", 0)
}
...
// FakeClient
type FakeEhtClient struct {
	MockEthClient
	numHeaderByNumberCalls uint16
	NewHeaders             []*types.Header
	WaitGroup              *sync.WaitGroup
}

func (f *FakeEhtClient) SubscribeNewHead(
	ctx context.Context,
	ch chan<- *types.Header,
) (ethereum.Subscription, error) {
	go func() {
