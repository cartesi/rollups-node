...
func (s *Service) recordTournamentObservationFailure(app *model.Application, head, windowEnd uint64, err error) {
	if s.IsStopping() && errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
		return
	}
