...
func (s *LoadSuite) TestOkAccept() {
	...
	machine, err := Load(s.acceptSnapshot.Dir, s.address, config)
	require.Nil(err)
...
func (s *LoadSuite) TestOkReject() {
	require := s.Require()
	config := &emulator.MachineRuntimeConfig{}
	machine, err := Load(s.rejectSnapshot.Dir, s.address, config)
	require.Nil(err)
	require.NotNil(machine)
}
...
func (s *LoadSuite) TestInvalidAddress() {
	require := s.Require()
	config := &emulator.MachineRuntimeConfig{}
	machine, err := Load(s.acceptSnapshot.Dir, "invalid-address", config)
...
func (s *LoadSuite) TestNotAtManualYield() {
	require := s.Require()
	config := &emulator.MachineRuntimeConfig{}
	machine, err := Load(s.noticeSnapshot.Dir, s.address, config)
	require.NotNil(err)
...
func (s *LoadSuite) TestException() {
	require := s.Require()
	config := &emulator.MachineRuntimeConfig{}
	machine, err := Load(s.exceptionSnapshot.Dir, s.address, config)
	require.NotNil(err)
...
func (s *ForkSuite) TestOk() {
	...
	// Loads the machine.
	machine, err := Load(snapshot.Dir, address, &emulator.MachineRuntimeConfig{})
	require.Nil(err)
	require.NotNil(machine)
...
func (s *AdvanceSuite) TestEchoLoop() {
	...
	// Loads the machine.
	machine, err := Load(s.snapshotEcho.Dir, s.address, &emulator.MachineRuntimeConfig{})
	require.Nil(err)
	require.NotNil(machine)
	defer func() { require.Nil(machine.Close()) }()
...
func (s *AdvanceSuite) TestAcceptRejectException() {
	...
	// Loads the machine.
	machine, err := Load(snapshot.Dir, s.address, &emulator.MachineRuntimeConfig{})
	require.Nil(err)
	require.NotNil(machine)
	defer func() { require.Nil(machine.Close()) }()
...
func (s *CyclesSuite) SetupSubTest() {
	...
	s.machine, err = Load(s.snapshot.Dir, s.address, &emulator.MachineRuntimeConfig{})
	require.Nil(err)
