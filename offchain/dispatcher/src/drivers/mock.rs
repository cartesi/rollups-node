...
pub enum Event {
    ...
    FinishEpoch(u64), // index of the "external" epoch
}
...
impl BrokerSend for Broker {
    ...
    async fn finish_epoch(&self, _: u64) -> Result<(), BrokerFacadeError> {
        if self.finish_epoch_error {
            ...
            let mut mutex_guard = self.events.lock().unwrap();
            let current_epoch = self.finish_epochs.load(Ordering::SeqCst);
            mutex_guard
                .deref_mut()
                .push(Event::FinishEpoch(current_epoch.into()));
            self.finish_epochs
                .store(current_epoch + 1, Ordering::SeqCst);
            Ok(())
        }
    }
}
