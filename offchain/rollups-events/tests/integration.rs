...
async fn test_it_consumes_from_multiple_streams() {
    ...
    // Sets a thread to produce an event in that stream after WAIT seconds.
    const WAIT: u64 = 2;
    let mut producer_broker = broker.clone();
    let handler = tokio::spawn(async move {
        let duration = Duration::from_secs(WAIT);
        tokio::time::sleep(duration).await;
        let data = "final event".to_string();
        let payload = MockPayload { data };
        let _ = producer_broker
            .produce(&stream, payload)
            .await
